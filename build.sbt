...
val akka = "com.typesafe.akka" %% "akka-actor" % "2.5.+" % "provided"
val akkaHttp = "com.typesafe.akka" %% "akka-http" % "10.1.+"
...
val ocppSpray = module("ocpp-spray")
  .dependsOn(ocppSoap)
  .settings(
    scalaVersion := tnm.ScalaVersion.prev, // overriding it as spray is only build for 2.11
    crossScalaVersions := Seq(tnm.ScalaVersion.prev),
    libraryDependencies ++= Seq(
      sprayHttp, sprayHttpX, akka, specs2Mock))

val occpAkkaHttp = module("ocpp-akka-http")
  .dependsOn(ocppSoap)
  .settings(
    libraryDependencies ++= Seq(
      akkaHttp,  specs2Mock)) 
