...
import com.thenewmotion.ocpp.messages.{CentralSystemReq => CsReq, CentralSystemRes => CsRes, ChargePointReq => CpReq, ChargePointRes => CpRes, _}
...
class OcppProcessingSpec extends Specification with Mockito with SoapUtils {
  ...
  private trait TestScope extends Scope {
    ...
    implicit class RichResponseEntity(val entity:ResponseEntity) extends AnyVal{
      def asString={
    ...
    val httpRequest = HttpRequest(
      HttpMethods.POST,
      Uri("/"),
      List(`Content-Type`(MediaTypes.`application/soap+xml` withCharset HttpCharsets.`UTF-8`),
        `X-Forwarded-For`(RemoteAddress.apply(InetAddress.getByName("192.168.1.100")))),
